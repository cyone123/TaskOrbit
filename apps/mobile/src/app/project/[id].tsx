    ...
    >
      <PageScroll key={section}>
        <Card variant="filled" style={[styles.hero, { backgroundColor: colors.surfaceContainerHigh }]}>
          ...
          <View style={styles.heroProgressLabel}><Text style={[styles.heroProgressText, { color: colors.onSurface }]}>整体进度</Text><Text style={[styles.heroProgressValue, { color: accent }]}>{metrics.progress}%</Text></View>
          <ProgressBar value={metrics.progress} color={accent} />
          <Text style={[styles.heroProgressMeta, { color: colors.onSurfaceVariant }]}>{metrics.taskTotal > 0 ? `${metrics.taskDone} / ${metrics.taskTotal} 个任务完成` : `${metrics.planDone} / ${metrics.planTotal} 个计划完成`}</Text>
        </Card>
